Author: Shahar Hikri, ID: 313237703
...
MutualLogicFuncs.h:
mutual logic functions which are mutual to CudaFucs.cu (cuda sub system) and to MPI&OpenMP sub system (LogicFuncs.c).
-functions have same implementation - but different names and definitions - '__device__' prefix in cuda.

dicionaries.h:
* has created by the program 'dictionaries_h_creation.py' (using python).
global arrays (dictionaries) also difine differently in cuda ('__device__' prefix).
